        ...
        for tab, prop, fname in zip(tabs, proposals_list, file_names):
            with tab:
                ...
                with col3:
                    ...
                    prop["wellness_screening"] = st.text_input("Wellness Screening",   prop.get("wellness_screening", ""), key=f"well_{fname}")
                    prop["cancer_screening"]   = st.text_input("Cancer Screening",     prop.get("cancer_screening", ""),   key=f"canscr_{fname}")
                    prop["organ_transplant"]   = st.text_input("Μεταμόσχευση",         prop.get("organ_transplant", ""),   key=f"organ_{fname}")
                    prop["hospice_care"]       = st.text_input("Ανακουφιστική Φρ.",    prop.get("hospice_care", ""),       key=f"hosp2_{fname}")
